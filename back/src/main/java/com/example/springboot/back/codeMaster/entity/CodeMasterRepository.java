...
public interface CodeMasterRepository extends JpaRepository<CodeMaster,String>{
    ...
    @Query(value = "select * from TS_CODE_MASTER c where c.class_code like %:searchKeyword% or c.class_name like %:searchKeyword%" ,nativeQuery = true)
    ...
    @Query(value = "select * from TS_CODE_MASTER c where c.class_name like %:searchKeyword%" ,nativeQuery = true)
